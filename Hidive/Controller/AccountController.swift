    ...
    func loadDashboard(bucketsCount: Int = 10, entriesPerBucketCount: Int = 25) async {
        do {
            ...
            var result: DashboardResponse = try await apiController.sendRequest(
                method: "GET",
                path: "v4/content/home?bpp=\(bucketsCount)&rpp=\(entriesPerBucketCount)&displaySectionLinkBuckets=SHOW&displayEpgBuckets=HIDE&displayEmptyBucketShortcuts=SHOW&displayContentAvailableOnSignIn=SHOW&displayGeoblocked=SHOW&bspp=\(bucketsCount)"
            )
