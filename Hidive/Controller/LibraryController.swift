...
class LibraryController {
    ...
    func getWatchlist(id: Int, attributed: Bool, from: Watchlist? = nil) async throws -> Watchlist {
        let lastSeen = if let paging = from?.paging {
        ...
        let result: Watchlist = try await apiController.sendRequest(
            method: "GET",
            path: "\(attributed ? "v4" : "v3")/user/watchlist/\(id)?\(lastSeen)rpp=10"
        )
        if(attributed) {
            try await attributeWatchlist(watchlist: result)
            if let previousContent = from?.content {
                result.content?.insert(contentsOf: previousContent, at: 0)
            }
        }
    ...
    func createWatchlist(name: String) async throws {
        ...
        if case .success(let watchlists) = watchlists {
            let watchlist = try await getWatchlist(id: response.id, attributed: false)
    ...
    func addWatchlistItem(watchlist: Watchlist, whatchlistEntry: DescriptableEntry) async throws {
        ...
        if var content = watchlist.content {
            if content.isEmpty, let thumbnail = whatchlistEntry.coverUrl {
                watchlist.thumbnails = [thumbnail]
            }
            
            content.append(whatchlistEntry)
        }else {
            let attributedWatchlist = try await getWatchlist(id: watchlist.id, attributed: true)
            watchlist.thumbnails = attributedWatchlist.thumbnails
            watchlist.content = attributedWatchlist.content
        }
    }
