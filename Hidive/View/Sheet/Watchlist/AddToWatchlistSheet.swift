    ...
    var body: some View {
        NavigationStack {
            ...
            .toast(isPresenting: $loadingToast) {
                AlertToast(type: .loading, title: "Adding item to watchlist...")
            }
            .toast(isPresenting: $errorToast) {
                if let requestError = error as? RequestError, case .invalidResponseStatusCode(let statusCode) = requestError, statusCode == 409 {
                    AlertToast(type: .error(Color.red), title: "Cannot add item to watchlist", subTitle: "Duplicated item")
                }else {
                    AlertToast(type: .error(Color.red), title: "Cannot add item to watchlist", subTitle: error?.localizedDescription ?? "Unknown error")
                }
            }
        }
    }
