...
private let scrollableCoordinateSpace = "scrollable"
...
struct SeriesView: View {
    ...
    var body: some View {
        GeometryReader { outerGeometry in
            ScrollView {
                scrollableBody(outerGeometry: outerGeometry).background(
            ...
            SeriesToolbarButtonView(
                ...
                action: {
                    if case .success(let data) = holder {
                        self.addToWatchlistItem = data.episodable.descriptableEntry
                    }
                }
            )
        }
    ...
    @ViewBuilder
    private func scrollableBody(outerGeometry: GeometryProxy) -> some View {
        switch(holder) {
    ...
    private func loadedBody(data: Holder) -> some View {
        LazyVStack(alignment: .leading, spacing: 0) {
            ...
            .frame(maxWidth: .infinity, minHeight: thumbnailHeight)
            .opacity(thumbnailOpacity)
            .id("\(data.episodable.id)_thumbnail")
