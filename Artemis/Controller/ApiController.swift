...
final class ApiController {
    ...
    func sendRequest<T : Decodable>(method: String, url: String? = nil, path: String = "", data: Encodable? = nil, additionalHeaders: [String:String]? = nil, log: Bool = false, requiresLogin: Bool = true) async throws -> T {
        ...
        do {
            if let generalResponse: GeneralResponse = try? decoder.decode(GeneralResponse.self, from: response), let statusCode = generalResponse.code {
                if(!String(statusCode).starts(with: "20")) {
                    throw RequestError.invalidResponseStatusCode(statusCode, generalResponse.message)
                }
            }
